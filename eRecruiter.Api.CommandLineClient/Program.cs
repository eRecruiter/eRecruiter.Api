...
namespace eRecruiter.Api.CommandLineClient
{
    class Program
    {
        ...
        private static void Main(string[] args)
        {
            if (CommandLine.Parser.Default.ParseArguments(args, Options))
            {
                ...
                var client = new ApiHttpClient(new Uri(Options.ApiUrl),
                    () => new ApiKeyParameter { ClientInfo = "CommandLineClient", Key = Options.ApiKey, MandatorId = Options.MandatorId },
                    () => tokenCache);
        ...
        static async Task Ping(ApiHttpClient client)
        {
        ...
        static async Task CvParser(ApiHttpClient client)
        {
        ...
        private static async Task Mandator(ApiHttpClient client)
        {
            ...
            Console.WriteLine("Knowledge tree loaded ({0} jobProfiles), {1} ms.", knowledgeTree.JobProfiles.Count(), client.ElapsedMillisecondsInLastCall);
        }
        ...
        private static async Task Jobs(ApiHttpClient client)
        {
            ...
            if (questionnaireStopwatch.ElapsedMilliseconds <= 0 || questionnaireCount <= 0)
                ...
                Console.WriteLine("{0} questionnaires loaded with {1} items total, {2}ms average per questionnaire.", questionnaireCount, questionnaireItemsCount, questionnaireStopwatch.ElapsedMilliseconds / questionnaireCount);
        }

        private static async Task Applicant(ApiHttpClient client)
        {
            foreach (var applicantId in (Options.ApplicantId ?? "").Split(',').Select(x => x.Trim()).Where(x => x.IsInt()).Select(x => x.GetInt()))
            {
                ...
                Console.WriteLine("Applicant '{0}', {1}ms.", applicant.FirstName + " " + applicant.LastName, client.ElapsedMillisecondsInLastCall);

                var savedApplicant = await new ApplicantPostRequest(applicant.Id, applicant).LoadResultAsync(client);
                Console.WriteLine("Updated Applicant '{0}', {1}ms.", savedApplicant.FirstName + " " + savedApplicant.LastName, client.ElapsedMillisecondsInLastCall);

                var documents = await new ApplicantDocumentsGetRequest(applicantId).LoadResultAsync(client);
                Console.WriteLine("{0} documents loaded, {1}ms.", documents.Count(), client.ElapsedMillisecondsInLastCall);

                var activities = await new ApplicantActivitiesGetRequest(applicantId).LoadResultAsync(client);
                Console.WriteLine("{0} activities loaded, {1}ms.", activities.Count(), client.ElapsedMillisecondsInLastCall);

                var publications = await new ApplicantPublicationsGetRequest(applicantId).LoadResultAsync(client);
                Console.WriteLine("{0} publications loaded, {1}ms.", publications.Count(), client.ElapsedMillisecondsInLastCall);

                var certificates = await new ApplicantCertificatesGetRequest(applicantId).LoadResultAsync(client);
                Console.WriteLine("{0} certificates loaded, {1}ms.", certificates.Count(), client.ElapsedMillisecondsInLastCall);

                var educations = await new ApplicantEducationsGetRequest(applicantId).LoadResultAsync(client);
                Console.WriteLine("{0} educations loaded, {1}ms.", educations.Count(), client.ElapsedMillisecondsInLastCall);

                var experiences = await new ApplicantExperiencesGetRequest(applicantId).LoadResultAsync(client);
                Console.WriteLine("{0} experiences loaded, {1}ms.", experiences.Count(), client.ElapsedMillisecondsInLastCall);

                var languages = await new ApplicantLanguagesGetRequest(applicantId).LoadResultAsync(client);
                Console.WriteLine("{0} languages loaded, {1}ms.", languages.Count(), client.ElapsedMillisecondsInLastCall);

                var customFields = (await new ApplicantCustomFieldsGetRequest(applicantId).LoadResultAsync(client)).ToList();
                Console.WriteLine("{0} custom fields loaded, {1}ms.", customFields.Count(), client.ElapsedMillisecondsInLastCall);

                var knowledges = (await new ApplicantKnowledgesGetRequest(applicantId).LoadResultAsync(client)).ToList();
                Console.WriteLine("{0} knowledges loaded, {1}ms.", knowledges.Count(), client.ElapsedMillisecondsInLastCall);

                var knowledge = (await new ApplicantKnowledgeGetRequest(applicantId, knowledges.First().Knowledge).LoadResultAsync(client));
                Console.WriteLine("Knowledge {0} loaded, {1}ms.", knowledge.Knowledge, client.ElapsedMillisecondsInLastCall);
                ...
                {
                    if (customField.CustomField.Type == CustomFieldResponse.CustomFieldType.TextMultiLine || customField.CustomField.Type == CustomFieldResponse.CustomFieldType.TextSingleLine)
                        await new ApplicantCustomFieldPostRequest(applicantId, new CustomFieldValueParameter { CustomField = customField.CustomField.Name, Value = Guid.NewGuid().ToString() }).LoadResultAsync(client);

                    await new ApplicantCustomFieldGetRequest(applicantId, customField.CustomField.Name).LoadResultAsync(client);
                    await new ApplicantCustomFieldDeleteRequest(applicantId, customField.CustomField.Name).LoadResultAsync(client);
                }
                ...
                await new ApplicantCoverLetterPutRequest(applicantId, new ApplicantCoverLetterCreateParameter(null, "Some cover letter")).LoadResultAsync(client);
                Console.WriteLine("{0}, {1}ms.", "Cover Letter added", client.ElapsedMillisecondsInLastCall);

                await new ResetPasswordRequest(applicantId, new ApplicantResetPasswordParameter("asdf", null)).LoadResultAsync(client);
                ...
                Console.WriteLine("Applicant '{0}' loaded by credentials, {1} ms.", applicant.FirstName + " " + applicant.LastName, client.ElapsedMillisecondsInLastCall);

                try
                {
                    applicant = await new ApplicantGetRequest(applicant.Id, "STZVJj41PGcGIsFxpHfQvxtRCtoAjG").LoadResultAsync(client);
                    Console.WriteLine("Applicant '{0}' loaded by logon token, {1} ms.", applicant.FirstName + " " + applicant.LastName, client.ElapsedMillisecondsInLastCall);
                }
                catch
                {
                    Console.WriteLine("Unable to load applicant by logon token, {0} ms.", client.ElapsedMillisecondsInLastCall);
                }
            }
        }
